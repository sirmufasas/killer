...
Math.sin(a + x * 35 * Math.PI / 180) * p.s / 2, p.s / 4, 0, 2 * Math.PI);
      ...
      if (gameover) {
        ...
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText("WUUHDUUUUP! " + (player.zombiesKilled < 1 ? "No" :
 player.zombiesKilled.toLocaleString()) + " Thugs" +
 (player.zombiesKilled == 1 ? "" : "s") + 
" Killed", can.width / 2, can.height / 2);
        ctx.font = "50px creepster";
        ctx.fillText("PRESS ENTER TO TOAST EM NIGGAS!", can.width / 
2, 0.75 * can.height);
      } else {
...
player.spray + "/" + player.magSize /
 player.spray, 0, 40);
  ...
  player.x = Math.max(player.s, Math.min(can.width - player.s, player.x));
  player.y = Math.max(player.s, Math.min(can.height - player.s, player.y));
